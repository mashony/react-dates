...
import CalendarYear, { getModifiersForMonth } from '../../src/components/CalendarYear';
...
describe('CalendarYear', () => {
  describe('#render', () => {
    ...
    describe('caption', () => {
      ...
      it('text is the correctly formatted year title', () => {
        const yearThunk = () => moment();
        const caption = shallow(<CalendarYear year={yearThunk} />).find('caption');
        expect(caption.text()).to.equal(yearThunk().format('YYYY'));
      });
    });
  ...
  describe('#getModifiersForMonth', () => {
    ...
    it('returns key for true modifier', () => {
      ...
      const filteredModifiers = getModifiersForMonth(modifiers, moment());
